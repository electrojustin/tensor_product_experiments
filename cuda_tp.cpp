...
torch::Tensor tensor_product_forward(torch::Tensor in1, torch::Tensor in2,
		                     torch::Tensor channel_weight,
                                     torch::Tensor out,
                                     torch::Tensor cb_palette,
                                     torch::Tensor block_jobs,
                                     torch::Tensor block_job_sizes) {
  float* channel_weight_ptr = channel_weight.size(0) != 0 ? channel_weight.data<float>() : NULL;
  tensor_product_forward_cuda(
      in1.data<float>(), in2.data<float>(), channel_weight_ptr,
      out.data<float>(),
